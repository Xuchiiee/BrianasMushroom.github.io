<?php
	...
	if(isset($_SESSION['manufacturer_login'])) {
		if(isset($_SESSION['manufacturer_login']) == true) {
			...
			$query_selectInvoice_Id = "SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='scm_new' AND TABLE_NAME='invoice'";
			$result_selectInvoice_Id = mysqli_query($con, $query_selectInvoice_Id);
			$row_selectInvoice_Id = mysqli_fetch_array($result_selectInvoice_Id);

			// Check if the AUTO_INCREMENT key exists in the array
			if (isset($row_selectInvoice_Id['AUTO_INCREMENT'])) {
				$invoiceNumber = $row_selectInvoice_Id['AUTO_INCREMENT'];
			} else {
				$invoiceNumber = "N/A"; // Set a default value if the key is not found
			}
		} else {
	...
	<section>
		...
		<form action="insert_invoice.php" method="POST" class="form">
			...
			<table class="table_invoiceFormat">
				...
				<?php $i++; } ?>
				<?php mysqli_data_seek($result_selectOrderItems, 0); // Reset the result set pointer ?>
