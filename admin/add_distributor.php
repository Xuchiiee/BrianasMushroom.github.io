<?php
	...
	if(isset($_SESSION['admin_login'])) {
		if($_SESSION['admin_login'] == true) {
			...
			$nameErr = $emailErr = $phoneErr = $requireErr = $passwordErr = $confirmMessage = "";
			...
			if($_SERVER['REQUEST_METHOD'] == "POST") {
				...
				if(!empty($_POST['txtDistributorpassword'])) {
					$passwordHolder = $_POST['txtDistributorpassword'];
					$resultValidate_password = validate_email($_POST['txtDistributorpassword']);
					if($resultValidate_password == 1) {
						$password = $_POST['txtDistributorpassword'];
					}
					else {
				...
				if($name != null && $phone != null && $resultValidate_email ==1) {
					$query_addDistributor = "INSERT INTO distributor(dist_name,password,dist_email,dist_phone,dist_address) VALUES('$name','$password','$email','$phone','$address')";
...
<body>
	...
	<section>
		...
		<li>
			<div class="label-block"> <label for="distributor:password">password</label> </div>
			<div class="input-box"> <input type="password" id="distributor:password" name="txtDistributorpassword" placeholder="password" value="<?php echo $passwordHolder; ?>" required /> </div> <span class="error_message"><?php echo $passwordErr; ?></span>
		</li>
		<li>
